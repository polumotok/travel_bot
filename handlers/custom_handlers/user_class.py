...
from handlers.custom_handlers import class_for_bot
...
class User:
    ...
    def city_search(self, message, letter: str):
        ...
        for city in class_for_bot.City(message.text).city_search():
            ...
            button = types.InlineKeyboardButton(
                city['call_name'],
                callback_data=f"{city['call_name'][0]}_{city['call_result']['destinationId']}_{city['call_result']['geoId']}_{letter}"
            )
            markup.row(button)
        if count > 0:
    ...
    def hotel_search(self, message, city_id: str):
        ...
        """
        with open(os.path.abspath('foto.json'), "r") as fh:
            foto = json.load(fh)
        sort_object = tuple()
        counter = 0
